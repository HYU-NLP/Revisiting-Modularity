...
def main():
    ...
    def read_example_data():
        data_dir = 'example_data/data_1' # or 'example_data/data_2'
        with open(f'{data_dir}/question.txt', 'r') as rf:
            question = rf.read()
        with open(f'{data_dir}/tests.json', 'r') as rf:
            tests = json.load(rf)
        with open(f'{data_dir}/corrected_solution.py', 'r') as rf:
            solution = rf.read()
        if os.path.exists(f'{data_dir}/starter_code.py'):
            with open(f'{data_dir}/starter_code.py') as rf:
                starter_code = rf.read()
        else:
            starter_code = ''
        return question, tests, solution, starter_code
    question, tests, solution, starter_code = read_example_data() 
