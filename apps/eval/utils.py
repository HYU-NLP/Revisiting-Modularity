...
def evaluate_generations(
    ...
    debug: bool = False,
    split: str = "test",
    column_name: str = "extracted_solutions",
):
    ...
    apps_eval = load_dataset(DATASET, split=split, trust_remote_code=True)

    ### Filter out the dataset to only include the websites we are interested in
    words = ["codeforces", "atcoder", "codechef"]
    apps_eval = apps_eval.filter(lambda x: any(word in x["url"] for word in words))

    if level == "all":
        pass
    elif level == "competition":
        data = data.filter(lambda x: x["difficulty"] == "competition")
        apps_eval = apps_eval.filter(lambda x: x["difficulty"] == "competition")
    elif level == "introductory":
        data = data.filter(lambda x: x["difficulty"] == "introductory")
        apps_eval = apps_eval.filter(lambda x: x["difficulty"] == "introductory")
    elif level == "interview":
        data = data.filter(lambda x: x["difficulty"] == "interview")
        apps_eval = apps_eval.filter(lambda x: x["difficulty"] == "interview")

    apps_eval = apps_eval.filter(lambda x: x["problem_id"] in data["problem_id"])
    results = {}
    for index in tqdm(range(len(data))):
        # code generations for problem (index)
        problem_generations = data[index]
        # get corresponding samples from APPS dataset
        sample = apps_eval[index]
        res = []
        # loop over the generations
        for o_idx, o in enumerate(problem_generations[column_name]):
            curr_res = [-2]
            try:
...
def get_results(
    results: Union[Dict[int, list], list],
    count_errors: bool = False,
    k_list: list = [1, 10, 100],
):
    """
    ...
    print("Computing pass@k metric for generations...")
...
def compute_metrics(
    ...
    results = evaluate_generations(
        data, level=level, debug=debug, split=split, column_name=column_name
    )
