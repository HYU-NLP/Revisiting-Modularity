...
def make_prompt(args, demonstration, test_data):
    if 'monolithic' in args.code_type:
        instruction = (
    ...
    elif 'modular' in args.code_type:
        instruction = (
            "Write a python code to solve the following coding problem "
            "that obeys the constraints and passes the example test cases. "
            "The output code needs to read from and write to standard IO. "
...
def main():
    ...
    file_name = f"{args.model.replace('/', '-')}_{args.num_icl_shot}shot_{args.num_gen}gen_{args.temperature}temp_{args.seed}seed_icl_result.jsonl"
    ...
    prompts = []
    # test_dataset = list(test_dataset)[:5] # for test
